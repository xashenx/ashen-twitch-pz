...
function performHandshake(EventsTable)
	...
	ServerEvent = {["Etype"] = "Handshake", ["ZedX"] = Zedx, ["ZedY"] = Zedy, ["ZedQ"] = zedquant, ["target"] = playerchar, ["username"] = playerChar:getUsername(), ["EventsTable"] = EventsTable}
...
function performEvent(module, command, result)
    if module ~= "AshenTwitchEvents" or command ~= "HandshakeResult" then
        return
    end
	print('------------=Twitch Events: HandshakeResult=------------', result["username"])
	print(playerChar:getUsername() .. "==" .. result["username"])
	if playerChar:getUsername() == result["username"] then
		print(result["EventsTable"])
	end
end

Events.OnServerCommand.Add(performEvent)
